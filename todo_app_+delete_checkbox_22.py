...
# ToDoリストのデフォルトデータ
todo_items = [["2025-12-31", "買い物", "牛乳を買う", False, "メモなし", 2]]
...
if os.path.exists(todo_file):
    with open(todo_file, "r", encoding="utf-8") as f:
        try:
            ...
            if isinstance(loaded_data, list) and all(isinstance(item, list) and len(item) == 6 for item in loaded_data):
...
# 優先度に応じた行の色を取得
def get_row_colors():
    return [(i, priority_colors.get(item[5], "white")) for i, item in enumerate(todo_items)]
...
def show_window():
    ...
    layout = [
        [sg.Frame(title="ToDo追加", layout=[
            ...
            [sg.Text("優先度:"), sg.Combo([1, 2, 3, 4], default_value=2, key="priority", size=(5,1))],
    ...
    win = sg.Window("ToDoアプリ", layout, font=("Arial", 14), resizable=True, size=(800, 500))
    ...
    while True:
        ...
        if event == "追加":
            ...
            new_item = [values["deadline"], values["tag"], values["input"], False, values["memo"], int(values["priority"])]
        ...
        if event == "更新":
            if edit_index is not None and edit_mode:
                todo_items[edit_index] = [values["deadline"], values["tag"], values["input"], False, values["memo"], int(values["priority"])]
                edit_mode = False
                edit_index = None
                win["input"].update("")
                win["memo"].update("")
